// Demo seed data: two checklist templates, a demo organisation + customer login, and one
// assessment per template so there's something to click through immediately.
//
// Idempotent — safe to re-run. Templates/organisations are matched by name and reused
// rather than duplicated; the customer user's password is reset each run so the printed
...
async function findOrCreateTemplate(name, category, sections, auditorId) {
  let template = await ChecklistTemplate.findOne({ name });
  if (template) {
    console.log(`Reusing existing template "${name}" (${template.sections.reduce((s, sec) => s + sec.questions.length, 0)} questions)`);
    return template;
  }
...
async function main() {
  ...
  const pciTemplate = await findOrCreateTemplate(
    'PCI DSS v4.0 Comprehensive Checklist',
    'Payment Security',
    pciSections(),
    auditor._id
  );
