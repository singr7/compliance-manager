...
<div id='msgShow'></div>
<div class="row">
<?php $status=Admin_status_change(ID_decode($_GET['service_id']),ID_decode($_GET['process_id']),$get_process_name->customer_id); ?>
    ...
    <div class="col-md-12">
        ...
        <div class="portlet light bordered">
            <div class="portlet-title">
				...
				<form action="<?php echo base_url(); ?>qsa/multiple_upload_qsa" method="POST" enctype="multipart/form-data">
		    <div class="row p-10">
					   <div class="col-lg-8 col-md-8 col-sm-8 col-xs-12">
                            ...
                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
								<div class="col-lg-11 col-md-11 col-sm-11 col-xs-11 p-10">
									<div class="portlet box red basic">
										...
										<div class="tab-content" style="width: 100%;float: left;clear: both; ">
											...
											<?php if(empty($data)){ echo "<p style='color: #697882;text-align:center;font-size:19px;'>No Data Uploaded.</p>"; }; ?>
										  ...
										  <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
											...
											if(isset($docsData) && !empty($docsData)){
													...
													?>
												<div class="col-md-4 portlet-body form-group mb-0">
								...
								<?php } } ?>
										...
										<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
											<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 p-5">
												<div class="col-md-4 portlet-body mb-0">
													<label class="control-label" style="color: #697882;"><strong> <?php if(isset($userInfo->full_name) && !empty($userInfo->full_name)){ echo  $Name[0];} ?> :</strong></label>
												</div>
												<div class="col-md-8 portlet-body">
													<textarea class="form-control" id="comment<?php echo $question->id;  ?>" name='comments[<?php echo $question->id;?>][]' rows="3" style="background-color: #f3f5f9;"></textarea>
												</div>
									...
									<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
										<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 p-5">
											...
											<div class="col-md-8 portlet-body">
											<input type="button" value="Reply"  class="btn green" style="padding: 8px 18px;margin-right:10px;" onClick="ReplyToComment(<?php echo $question->id;?>,<?php echo ID_encode($question->service_id);?>,<?php echo ID_encode($get_process_name->id);?>)">
											</div>
									...
									<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
										<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 p-10">
												...
												<div class="col-md-12 portlet-body">
													<input type="button" value="Accept & Assign to QA"  class="btn blue" style="    margin-bottom: 11px;"onClick="status(<?php echo ID_encode(1);?>,<?php echo ID_encode($question->id);?>,<?php echo ID_encode($question->service_id);?>,<?php echo ID_encode($get_process_name->id);?>)">
													<input type="button"   value="Disapproved"  class="btn red" style="margin-bottom: 11px;"  onClick="status(<?php echo ID_encode(2);?>,<?php echo ID_encode($question->id);?>,<?php echo ID_encode($question->service_id);?>,<?php echo ID_encode($get_process_name->id);?>)">
													<input type="button"  value="Mark Incomplete"  class="btn yellow" style="margin-bottom: 11px;" onClick="status(<?php echo ID_encode(4);?>,<?php echo ID_encode($question->id);?>,<?php echo ID_encode($question->service_id);?>,<?php echo ID_encode($get_process_name->id);?>)">
												</div>
												 <?php  }} ?>
										</div>
									</div>
							 ...
							 <?php if((trim(@$data[0]->status) == '0' || trim(@$data[0]->status) == '2' || trim(@$data[0]->status) == '4' ||  trim(@$data[0]->qa_status) == '2' || trim(@$data[0]->qa_status) == '4')){ 
								...
								<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
									<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
										<div class="col-md-8 portlet-body p-6 form-group no-padding">
										...
										<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 text-center" style="padding: 17px 10px 10px 10px;">
										...
										<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 no-padding dynamic_field<?php  echo $question->id; ?>">
										</div>
									</div>
								</div>
							 <?php  } } ?>
...
// start chnage the status
	function status(statusid,question_id,service_id,process_id){
		$.ajax({
			type:"POST",
			url:"<?php echo base_url();?>/qsa/updateStatusBy_Qsa",
			data:{statusid:statusid,question_id:question_id,service_id:service_id,process_id:process_id},
			success:function(success){
				//$('#msgShow').html(success);
				//alert(success);
				location.reload();
			}
		});
...
// start reply to comment to customer
	function ReplyToComment(question_id,service_id,process_id){
		var comment=$("#comment"+question_id).val();
		if(comment==""){
			...
			$.ajax({
				type:"POST",
				url:"<?php echo base_url();?>/qsa/updateCommentBy_Qsa",
...
// end reply to comment to customer	
	...
	// start delete file from  upload_evidence_docs  
	  function Delete_dcos(docs_id){
		  ...
		  $.ajax({
			  type:"POST",
			  url:"<?php echo base_url() ?>qsa/delete_qsa_docs",
		...
		function RequestForModification(id,question_id,service_id,process_id,customer_id){
			$.ajax({
				type:"POST",
				url:"<?php echo base_url();?>qsa/RequestForModification",
...
// add more file in upload evidence
	function AddMore(id){
		// event.preventDefault();
		    ...
		    var k=i+1;;
		    var j=id+k;
		   $('.dynamic_field'+id+'').append('<tr id="row'+i+'"><td style="padding: 5px 5px 5px 0px;"><input style="width:319px;" type="file" id="fileuplaod'+j+'" onchange="return fileValidation('+j+')" name="docs['+id+'][]" multiple="multiple" class="form-control name_list example" /></td><td><button type="button" name="remove" id="'+i+'" class="btn btn-danger btn_remove">X</button></td></tr>');  
	}	  
...
<script>
    $('.cancel-button').on('click', function () {
        url = '<?php echo base_url(); ?>qsa/dashboard';
        location = url;
    });
</script>