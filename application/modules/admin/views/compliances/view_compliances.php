...
<?php
 ...
 $status=Admin_status_change(ID_decode($_GET['service_id']),ID_decode($_GET['process_id']),ID_decode($_GET['cus_id']));
 $approvedData= Admin_status_change(ID_decode($_GET['service_id']),ID_decode($_GET['process_id']),ID_decode($_GET['cus_id']));
 ?>
        ...
        <div class="portlet light bordered">
            <div class="portlet-title">
                <div class="caption font-dark">
                    <span class="caption-subject bold uppercase"> View <?php if(isset($service_name->service_name) && !empty($service_name->service_name)){ echo $service_name->service_name;};?> Project (Compliance)</span>
					...
					<?php if(isset($start_date[0]->end_date) && !empty($start_date[0]->end_date)){ ?>
								<span class="caption-subject status"  style=
								'font-size: 14px;'> End Date:  <?php echo date("d-m-Y", strtotime($start_date[0]->end_date)); ?></span>
					<?php } ?>
                </div>
			...
			<form action="<?php echo base_url(); ?>admin/compliances/select_multiple_change_status" method="POST" enctype="multipart/form-data" id="report">
            <div class="row p-10">
					   <div class="col-lg-8 col-md-8 col-sm-8 col-xs-12">
							<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
								...
								<div class="">
										...
										<span style="margin-left: -77px;">All</span></div>
									</div>
							</div>
							<?php	} ?>
							</div>
							    <?php $datas=array();if(isset($questionair) && !empty($questionair)){
								  ...
								  $customer_id=ID_decode($_GET['cus_id']);
					...
					// $admin_log=admin_log(@$question->id,@$question->service_id,$process_id,$customer_id);
									...
									<div class="portlet box red basic">
										<div class="portlet-title">
											<div class="caption">
												...
												<input type="hidden" name="customer_id" value="<?php echo ID_decode($_GET['cus_id']); ?>">
										...
										<!-- start code for admin atatus-->
											...
											<!--<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
													<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 p-5">
													<?php if(isset($admin_log) && !empty($admin_log)){
														foreach($admin_log as $admin_logs){ 	?>
														<div class="col-md-12 portlet-body p-5">
															<label class="control-label" style="color: #697882;">
																<strong>Status: Modification Request To Accepted <?php echo date("d-m-Y H:i", strtotime($admin_logs->updated_date)); ?></strong>
															</label>
														</div>
													<?php } } ?>
                 	...
                 	<!-- New Row -->
								...
								<?php if(trim(@$approvedData) == "Approved"){
										...
										?>
					<div class="col-lg-4 col-md-4 col-sm-4 col-xs-12 text-center" style="padding-top: 60px; ">
								...
								<?php 
								 ...
								 ?>
									...
									<div class="col-md-12 portlet-body text-center no_border end_date">
										...
										<a href="javascript:void(0)" onClick="EndDate(<?php echo $_GET['id'].",".$_GET['cus_id'].",".$_GET['process_id'].",".$_GET['service_id'] ?>);">Go</a>
									...
									<div class="col-lg-8 col-md-8 col-sm-8 col-xs-12 p-10" >
										<div class="col-md-12 portlet-body text-center no_border">
											<div class="basic border-tab">
												<p class="m-0 portlet red p-5" style="color:#fff">ROC</p>
												<div class="col-md-12 portlet-body form-group">
													<label class="control-label" style="color: #697882;">
														<strong>
															<a  href='<?php echo base_url()?>uploads/report/<?php  echo $roc->report_docs; ?>'  data-toggle="tooltip" title="<?php echo $roc->report_docs;  ?>" download><?php  echo $file; ?></a>
															
															<button type='button' class='delete_btn' style='margin-right:10px;font-size:10px;' onClick='Delete_aoc_roc(<?php echo @ID_encode($roc->id).",".@ID_encode($roc->service_id).",".@ID_encode($roc->process_id).",".@ID_encode($roc->year).",".@ID_encode($roc->customer_id); ?>);'><span class='glyphicon glyphicon-trash'></span></button>
															<p class="mb-0 mt-5" style="color: #697882;"><?php  echo date("d-m-Y H:i", strtotime($roc->updated_date)); ?></p>
														</strong>
													</label>
												</div>
											</div>
								...
								<?php }else{ ?>
									...
									<div class="col-md-12 portlet-body text-center no_border">
										<input type="file" id="imguploadroc" onchange="validatePDF(this)" name="imguploadroc" style="display:none"/> 
											<input type="button" <?php  if(trim(@$approvedData) == "Approved"){ echo "";}else{echo "disabled";} ?> value="Upload ROC" id="OpenImgUploadROC"  class="btn  cust_btn" style="width:100%"><img id="loaderroc" class="loader" src="<?php echo base_url(); ?>/assets/pages/img/ajax-loader.gif"  style="display:none;">
									</div>
								</div>
								<?php	}	?>
								<?php if(!empty($aoc)){
									...
									<div class="col-lg-8 col-md-8 col-sm-8 col-xs-12 p-10" >
										<div class="col-md-12 portlet-body text-center no_border">
											<div class="basic border-tab">
												<p class="m-0 portlet red p-5" style="color:#fff">AOC</p>
											
												<div class="col-md-12 portlet-body form-group">
													<label class="control-label" style="color: #697882;">
														<strong>
															<a  href='<?php echo base_url()?>uploads/report/<?php  echo $aoc->report_docs; ?>' title="<?php echo $aoc->report_docs;  ?>" data-toggle="tooltip" download><?php  echo $file; ?></a>
															...
															<p class="mb-0 mt-5" style="color: #697882;"><?php  echo date("d-m-Y H:i", strtotime($aoc->updated_date)); ?></p>
														</strong>
													</label>
												</div>
											</div>
									...
									<div class="col-lg-8 col-md-8 col-sm-8 col-xs-12 p-10" >
										<div class="col-md-12 portlet-body text-center no_border">
												...
												<input type="button"  value="Upload AOC" id="OpenImgUploadAOC" <?php  if(trim(@$approvedData) == "Approved"){ echo "";}else{echo "disabled";} ?> name="OpenImgUploadAOC" class="btn blue-sharp cust_btn" style="width:100%"><img id="loaderaoc" class="loader"src="<?php echo base_url(); ?>/assets/pages/img/ajax-loader.gif"  style="display:none;"></button>
...
// start reply admin accept/reject to qa
	function AcceptToQa(id,question_id,service_id,customer_id,process_id){
			$.ajax({
				type:"POST",
				url:"<?php echo base_url();?>admin/compliances/accepttoqa",
				data:{accepted_id:id,question_id:question_id,service_id:service_id,customer_id:customer_id,process_id:process_id},
				success:function(success){
					//alert(success);
					 location.reload();
...
// start reply admin accept/reject to qa
	function AcceptToCustomer(id,question_id,service_id,customer_id,process_id){
			$.ajax({
				type:"POST",
				url:"<?php echo base_url();?>admin/compliances/accepttocustomer",
				data:{accepted_id:id,question_id:question_id,service_id:service_id,customer_id:customer_id,process_id:process_id},
				success:function(success){
					 location.reload();
				}
...
// start reply admin accept/reject to qa
	function AcceptToConsultant(id,question_id,service_id,customer_id,process_id){
			$.ajax({
				type:"POST",
				url:"<?php echo base_url();?>admin/compliances/AcceptToConsultant",
				data:{accepted_id:id,question_id:question_id,service_id:service_id,customer_id:customer_id,process_id:process_id},
				success:function(success){
					 location.reload();
				}
...
// start reply admin end date 
	function EndDate(project_id,customer_id,process_id,service_id){
		...
		if(date ==""){
			...
			$.ajax({
				type:"POST",
				url:"<?php echo base_url();?>admin/compliances/EndDateformAdmin",
...
$('#imguploadroc').trigger('click');
   $('#imguploadroc').on('change', function() {
	   if(formOK == true){
			var service_id=<?php echo $_GET['service_id']; ?>;
			var customer_id=<?php echo $_GET['cus_id']; ?>;
			...
			$.ajax({
				type:"POST",
				url:"<?php echo base_url();?>admin/compliances/FileUploadROC",
				data:  formData,
				contentType: false,
				cache: false,
				processData:false,
	...
	$('#imguploadaoc').trigger('click');
	    $('#imguploadaoc').on('change', function() {
			if(formOK == true){
				...
				$.ajax({
					type:"POST",
					url:"<?php echo base_url();?>admin/compliances/FileUploadAOC",
					data:  formData,
					contentType: false,
					cache: false,
					processData:false,
